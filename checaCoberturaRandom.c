...
//		if((buffer < LIMITE_BUFFER)&&(idPacote>=t0*tempoRtt)&&(idPacote<=t*tempoRtt)){
		...
		//if((buffer >= LIMITE_BUFFER)&&(idPacote>=t0*tempoRtt)&&(idPacote<=t*tempoRtt)){
if(buffer >= LIMITE_BUFFER){
			randomDescarte = rand()%LIMITE_BUFFER;	

			bufferMovel[randomDescarte].nodeID = idNoR;
			bufferMovel[randomDescarte].x = x;
			bufferMovel[randomDescarte].y = y;
			bufferMovel[randomDescarte].idPacote = idPacote;
	...
	for(buffer=0;buffer<LIMITE_BUFFER;buffer++){
	...
	printf("\nCobertura Sink em um Intervalo de Tempo com Buffer com descarte Random: %.2f \n",pCorbSinkBuffer);
