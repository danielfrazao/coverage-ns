...
if {$argc  == 5} {
    ...
    set opt(tempo_resched) [lindex $argv 3];		    ;# Tempo de envio de pacote
    set opt(ErroRssi) [lindex $argv 4]		            ;# Erro de Rssi
} else {
    debugMsg "Simulacao" "    Sintaxe: ns simulacao-cobertura.tcl <NumNos:156> <Densidade: 0.03> <NumTest:1> <Tempo_resched:0.5> <Erro_Rssi:0>" 
    debugMsg "Simulacao" "        Eg: ns simulacao-cobertura.tcl 156 0.03 1 > Saida.log";
    exit
}
...
for {set i 1} {$i < $opt(numNos) } {incr i} {
    ...
    # Cria o agente agenteFixo propriamente dito (antes foi a configuracao do no')
    set FixedAgent_($i) [new Agent/FixedAgent];             # Cria uma instanca do Protocolo FixedAgent
    $ns_ attach-agent $node_($i) $FixedAgent_($i);	    # Atribui como sendo um no' do FixedAgent
